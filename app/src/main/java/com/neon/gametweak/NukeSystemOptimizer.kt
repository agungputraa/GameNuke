...
object NukeSystemOptimizer {
    ...
    suspend fun applyGamingOptimizations(context: Context): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            // ── 4. OEM THROTTLE GUARD & GAMING RESTRICTION BYPASS ─────────
            when {
                mfr.contains("xiaomi") || mfr.contains("redmi") || mfr.contains("poco") || mfr.contains("blackshark") -> {
                ...
                mfr.contains("oppo") || mfr.contains("realme") || mfr.contains("oneplus") -> {
                    ...
                    sb.appendLine("cmd appops set com.oplus.cosa GET_USAGE_STATS ignore 2>/dev/null")
                }
                mfr.contains("vivo") || mfr.contains("iqoo") -> {
                    sb.appendLine("cmd appops set com.vivo.gamewatch GET_USAGE_STATS ignore 2>/dev/null")
                    sb.appendLine("cmd appops set com.vivo.gamecube GET_USAGE_STATS ignore 2>/dev/null")
                }
                mfr.contains("transsion") || mfr.contains("infinix") || mfr.contains("tecno") || mfr.contains("itel") -> {
                    sb.appendLine("cmd appops set com.transsion.neopower GET_USAGE_STATS ignore 2>/dev/null")
                    sb.appendLine("cmd appops set com.transsion.gameassistant GET_USAGE_STATS ignore 2>/dev/null")
                }
                mfr.contains("huawei") || mfr.contains("honor") -> {
                    sb.appendLine("cmd appops set com.huawei.powergenie GET_USAGE_STATS ignore 2>/dev/null")
                    sb.appendLine("cmd appops set com.hihonor.powergenie GET_USAGE_STATS ignore 2>/dev/null")
                }
                ...
                mfr.contains("motorola") || mfr.contains("lenovo") -> {
                    sb.appendLine("cmd appops set com.motorola.gamemode GET_USAGE_STATS ignore 2>/dev/null")
                }
                mfr.contains("redmagic") || mfr.contains("nubia") -> {
                    sb.appendLine("cmd appops set cn.nubia.gamelauncher GET_USAGE_STATS ignore 2>/dev/null")
                }
                mfr.contains("sony") -> {
                    sb.appendLine("cmd appops set com.sonymobile.gameenhancer GET_USAGE_STATS ignore 2>/dev/null")
                }
            }
            ...
            Log.e(TAG, "applyGamingOptimizations error handled safely: ${t.message}", t)
            false
        }
    }
    ...
    suspend fun restoreSystemDefaults(context: Context): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            Log.e(TAG, "restoreSystemDefaults error handled safely: ${t.message}", t)
            false
        }
    }
}
