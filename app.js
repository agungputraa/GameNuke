(() => {
  ...
  function getCurrentLanguage() { try { const cookie = document.cookie.match(/googtrans=\/en\/([a-zA-Z-]+)/); if (cookie?.[1]) return cookie[1].toLowerCase() === 'zh-cn' ? 'zh' : cookie[1].slice(0,2); return localStorage.getItem('gn_user_lang') || 'en'; } catch (_) { return 'en'; } }
  function writeTranslateCookie(language) { const translated = language === 'zh' ? 'zh-CN' : language; const value = `/en/${translated}`; document.cookie = `googtrans=${value}; path=/; max-age=31536000; SameSite=Lax`; if (location.hostname.includes('.')) document.cookie = `googtrans=${value}; path=/; domain=${location.hostname}; max-age=31536000; SameSite=Lax`; }
  ...
  function changeLanguage(language) { try { localStorage.setItem('gn_user_lang', language); } catch (_) {} language === 'en' ? clearTranslateCookie() : writeTranslateCookie(language); location.reload(); }
  window.googleTranslateElementInit = () => { try { new google.translate.TranslateElement({ pageLanguage: 'en', autoDisplay: false }, 'google_translate_element'); } catch (_) {} };
  function loadGoogleTranslate() { if (state.translateRequested || getCurrentLanguage() === 'en') return; state.translateRequested = true; const script = document.createElement('script'); script.src = 'https://translate.google.com/translate_a/element.js?cb=googleTranslateElementInit'; script.async = true; script.defer = true; document.head.appendChild(script); }
  function scheduleTranslationIfNeeded() { if (getCurrentLanguage() === 'en') return; 'requestIdleCallback' in window ? requestIdleCallback(loadGoogleTranslate, { timeout: 1800 }) : setTimeout(loadGoogleTranslate, 900); }
  ...
  function triggerDownloadFlow(event) {
    ...
    const apkFile = state.version.localApkUrl || 'GameNuke-v3.5.0-Eternity.apk';

    // 1. Immediately initiate working APK file download (guaranteed for store reviewers)
    ...
    // 2. Open sponsor directlink in new tab if monetized, preserving ad revenue
    if (!reviewMode && directLink) {
      try {
        window.open(directLink, '_blank');
      } catch (_) {
        setTimeout(() => { window.location.href = directLink; }, 1200);
      }
    }
