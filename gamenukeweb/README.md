...
<p align="center">
  <img src="https://img.shields.io/badge/Release-v2.2.0--prem-00ff88?style=for-the-badge&logo=android&logoColor=black" alt="Release Version">
...
- 📦 **Latest GitHub Release:** [Releases v2.2.0-prem](https://github.com/agungputraa/GameNuke/releases/latest)
...
### Automation Sequence:
1. **Build:** Compiles signed release APK (`com.neon.gametweak`) using `agwallpaper84.jks`.
2. **Verification:** Validates APK size and generates SHA-256 integrity hash.
3. **Metadata Sync:** Automatically updates `version.json` with new version details, download URL, and hash.
4. **Web Isolation & Push:** Isolates `gamenukeweb` files and this `README.md`, then force-pushes exclusively to `main` and `gh-pages`.
5. **GitHub Release:** Calls GitHub REST API with the token in `tools/github_token.env` to create the release tag.
6. **Binary Asset Upload:** Uploads `GameNuke-Premium-vX.X.X.apk` to the release assets.
...
- **Safety Guarantee:** Game Nuke operates strictly through official Android APIs (`VpnService`, `AccessibilityService`, and `Shizuku`). It does not modify game files, memory addresses, or server-side game data.
